...
class Logger
{
	...
	/**
	 * @var array instances of idea\log\Log
	 */
	 ...
	 * @throws ErrorException 如果设置的日志操作类不是idea\log\Log的子类，抛出异常
	 */
		...
		foreach ($writers as $writer) {
			if (!is_object($writer)) {
				throw new ErrorException(
					'Logger writer "%s" must be a instance of idea\log\Log', $writer
				);
			}
			
			if (!$writer instanceof Log) {
				throw new ErrorException(
					'Logger writer "%s" must be a instance of idea\log\Log', get_class($writer)
				);
			}
		}
