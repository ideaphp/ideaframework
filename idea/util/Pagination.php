 ...
 * @version $Id: Pagination.php 1 2013-03-29 16:48:06Z huan.song $
 * @package idea.util
 * @since 1.0
 */
class Pagination
{
	 ...
	 */
	public function __construct($totalRows, $url = '?&page=', $pageVar = 'page')
	{
	...
	public function getLinks(array $pages)
	{
		...
		for ($page = $links['first']; $page <= $links['last']; $page++) {
			$links['list'] = ($page == $links['curr']) ? $page : $this->getUrl($page);
		}
		return $links;
	}
	...
	public function getPages()
	{
		...
		$pages['first'] = $pages['curr'] - $pages['list'] / 2;
		if ($pages['first'] < 1) {
			$pages['first'] = 1;
		}
	...
	public function setListRows($listRows)
	{
		if (($listRows = (int) $listRows) <= 0) {
			throw new ErrorException(sprintf(
				'Pagination list rows "%d" can not less or equal than 0.', $listRows
			));
		}
	...
	public function setListPages($listPages)
	{
		if (($listPages = (int) $listPages) <= 0) {
			throw new ErrorException(sprintf(
				'Pagination list pages "%d" can not less or equal than 0.', $listPages
			));
	 	}
	...
	public function setTotalRows($totalRows)
	{
		if (($totalRows = (int) $totalRows) < 0) {
			throw new ErrorException(sprintf(
				'Pagination total rows "%d" can not less than 0.', $totalRows
			));
		}
	...
	public function setPageVar($pageVar)
	{
		...
		if (($pageVar = trim($pageVar)) == '') {
			throw new ErrorException(
				'Pagination set page var failed, pageVar must be string and not empty.'
			);
		}
